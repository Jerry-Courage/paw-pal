...
class GeminiLiveConsumer(AsyncWebsocketConsumer):
    ...
    async def run_live_session(self):
        ...
        try:
            ...
            logger.info(f"[LiveTrace] Connecting to {model_id}...")
            async with self.client.aio.live.connect(model=model_id, config=config) as session:
                self.gemini_session = session
                logger.info(f"[LiveTrace] Session HANDSHAKE SUCCESS with {model_id}")

                async for message in session.receive():
                    response = {}
                    if message.text:
                        logger.info(f"[LiveTrace] Gemini Text: {message.text}")
                    ...
                    if message.server_content and message.server_content.model_turn:
                        ...
                        for part in parts:
                            if part.inline_data:
                                logger.info(f"[LiveTrace] Gemini Audio Chunk Received ({len(part.inline_data.data)} bytes)")
            ...
            logger.info("[LiveTrace] Session Task Cancelled")
        except Exception as e:
            error_trace = traceback.format_exc()
            logger.error(f"[LiveTrace] Session Error: {e}\n{error_trace}")
            await self.close()
    ...
    async def receive(self, text_data=None, bytes_data=None):
        ...
        try:
            if text_data:
                ...
                # Client sending text
                if "text" in data:
                    logger.info(f"[LiveTrace] Client Text: {data['text']}")
