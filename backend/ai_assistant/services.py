    ...
    def generate_study_kit(self, resource, context: str = '', page_image_map: dict = None, vision_data: list = None, page_count: int = 0) -> dict:
        ...
        for idx, chunk_text in enumerate(chunks[:30]):
            ...
            prompt = (
                ...
                "      \"deep_dive\": \"[Deep academic details BUT keep it fun. Use bullet points, bold key terms. For formulas/math: BREAK INTO NUMBERED STEPS — show every intermediate calculation, explain each step in plain English, add 'Watch Out' for common mistakes, and do a sanity check at the end. Include 1-2 REAL-WORLD examples with humor. Make it click AND make them smile!]\",\n"
                ...
                "      \"ascii_art\": \"[An ASCII art diagram if the concept is visual — geometry, number lines, processes, flows, anatomy. Use box-drawing chars: ─ │ ┌ ┐ └ ┘ □ ■ ● ◆ → ← ↑ ↓. Make it clear and fun! Only if it adds value, otherwise empty string]\"\n"
                "    }\n"
                "  ],\n"
                ...
                "- MATH STEP-BY-STEP BREAKDOWN (CRITICAL — this is our superpower):\n"
                "  1. When ANY formula, equation, or calculation appears, break it into individual steps\n"
                "  2. Show EVERY step with numbers filled in: 'Step 1: Plug in... Step 2: Simplify... Step 3: Solve...'\n"
                "  3. After each step, explain WHAT just happened in plain English: 'We divided both sides by 2 to isolate x'\n"
                "  4. Use the mnemonic 'What, Why, Watch Out' for each step:\n"
                "     - WHAT: what are we doing? (e.g. 'Substitute the values')\n"
                "     - WHY: why this step? (e.g. 'We need to isolate x on one side')\n"
                "     - WATCH OUT: common mistake (e.g. 'Don't forget the negative sign!')\n"
                "  5. At the end, show the FINAL answer in a bold box: '**Answer: x = 42**'\n"
                "  6. Add a 'Sanity Check' — does the answer make sense? Plug it back in to verify.\n"
                "  7. If the formula has variables, FIRST explain what each variable means before using it.\n"
                ...
                "- ASCII ART: When the concept is VISUAL (geometry, number lines, cell anatomy, process flows, circuits, cycles), include an ascii_art field with a fun ASCII diagram. Use box-drawing chars (─ │ ┌ ┐ └ ┘), arrows (→ ← ↑ ↓), and shapes (□ ■ ● ◆). Example: a number line for fractions, a cell diagram, a water cycle. Only use when it genuinely helps — don't force it for abstract concepts.\n"
