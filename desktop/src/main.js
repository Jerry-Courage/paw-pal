...
function createWindow() {
  ...
  mainWindow.webContents.on('page-title-updated', (e, title) => {
    mainWindow.setTitle(title || 'Flow State')
  })
}
...
function createTray() {
  ...
  const contextMenu = Menu.buildFromTemplate([
    { label: 'Open Flow State', click: () => { mainWindow?.show(); mainWindow?.focus() } },
    { type: 'separator' },
...
function setupAutoUpdater() {
  ...
  autoUpdater.on('update-available', (info) => {
    dialog.showMessageBox(mainWindow, {
      ...
      message: `Flow State ${info.version} is available. Download now?`,
  ...
  autoUpdater.on('update-downloaded', () => {
    dialog.showMessageBox(mainWindow, {
      type: 'info',
      title: 'Update Ready',
      message: 'Update downloaded. Restart Flow State to apply it.',
