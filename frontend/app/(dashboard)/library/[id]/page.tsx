  ...
  return <main className="flow-v2 flow-atmosphere min-h-screen bg-flow-void px-5 pb-20 pt-8 text-flow-ink sm:px-8">
    <div className="mx-auto max-w-[76rem]">
      ...
      <header className="flex flex-col gap-5 border-b border-white/10 pb-8 sm:flex-row sm:items-start">
        ...
        <div className="min-w-0 flex-1"><p className="flow-eyebrow">{resource.resource_type || 'Source'}</p><h1 className="mt-2 max-w-4xl text-[clamp(1.9rem,3.5vw,3rem)] font-black leading-[1.06] tracking-[-.035em]">{resource.title}</h1><p className="mt-3 text-sm text-flow-muted">{[uploaded && `Uploaded ${uploaded}`, size].filter(Boolean).join(' · ')}</p></div>
...
function SourceAction({ href, icon, tone, label, detail }: { href: string; icon: React.ReactNode; tone: 'orange' | 'violet' | 'green'; label: string; detail: string }) { const color = tone === 'orange' ? 'text-flow-orange border-flow-orange' : tone === 'violet' ? 'text-flow-violet border-flow-violet' : 'text-flow-success border-flow-success'; return <Link href={href} className={`group flex min-h-36 flex-col justify-between border-l-4 bg-flow-raised p-4 ${color}`}><span className="[&>svg]:h-5 [&>svg]:w-5">{icon}</span><div className="mt-5"><h3 className="text-lg font-black text-flow-ink">{label}</h3><p className="mt-1.5 text-sm leading-5 text-flow-muted">{detail}</p><span className="mt-3 inline-flex items-center gap-1 text-sm font-black">Open <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></span></div></Link> }
