...
export default function AssignmentDetailPage({ params }: { params: { id: string } }) {
  ...
  const handleExport = async (fmt: 'pdf' | 'docx') => {
    ...
    const content = a?.ai_response || ''
    const subject = a?.subject || 'General'
    const safeFilename = title.replace(/[^\w\s-]/g, '').trim().replace(/\s+/g, '_').slice(0, 50)
    ...
    try {
      if (fmt === 'pdf') {
        ...
        for (const raw of lines) {
          ...
          // Strip markdown for pdf text (bold markers etc)
          const clean = line.replace(/\*\*(.*?)\*\*/g, '$1').replace(/\*(.*?)\*/g, '$1')
                            .replace(/^#{1,6}\s/, '')

          if (line.startsWith('### ')) {
            checkY(8); doc.setFont('helvetica', 'bold'); doc.setFontSize(11); doc.setTextColor(51, 65, 85)
            const t = doc.splitTextToSize(clean, maxW); doc.text(t, margin, y); y += t.length * 6 + 3
          } else if (line.startsWith('## ')) {
            checkY(10); doc.setFont('helvetica', 'bold'); doc.setFontSize(13); doc.setTextColor(30, 41, 59)
            const t = doc.splitTextToSize(clean, maxW); doc.text(t, margin, y); y += t.length * 7 + 4
          } else if (line.startsWith('# ')) {
            checkY(12); doc.setFont('helvetica', 'bold'); doc.setFontSize(16); doc.setTextColor(15, 23, 42)
            const t = doc.splitTextToSize(clean, maxW); doc.text(t, margin, y); y += t.length * 8 + 5
          } else if (line.startsWith('- ') || line.startsWith('* ')) {
            checkY(6); doc.setFont('helvetica', 'normal'); doc.setFontSize(10); doc.setTextColor(71, 85, 105)
            const bullet = `• ${clean.slice(2)}`
            const t = doc.splitTextToSize(bullet, maxW - 5); doc.text(t, margin + 3, y); y += t.length * 5.5 + 2
          } else {
            checkY(6); doc.setFont('helvetica', 'normal'); doc.setFontSize(10); doc.setTextColor(55, 65, 81)
            const t = doc.splitTextToSize(clean, maxW); doc.text(t, margin, y); y += t.length * 5.5 + 2
          }
        }
        ...
        const htmlLines = content.split('\n').map(line => {
          const l = line.trim()
          if (!l) return '<p></p>'
          const b = l.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/\*(.*?)\*/g, '<em>$1</em>')
          if (l.startsWith('### ')) return `<h3>${b.slice(4)}</h3>`
          if (l.startsWith('## ')) return `<h2>${b.slice(3)}</h2>`
          if (l.startsWith('# ')) return `<h1>${b.slice(2)}</h1>`
          if (l.startsWith('- ') || l.startsWith('* ')) return `<li>${b.slice(2)}</li>`
          return `<p>${b}</p>`
        }).join('\n')
        ...
        const html = `
          ...
          <style>body{font-family:Calibri,Arial;font-size:12pt;line-height:1.6;margin:2cm} h1{font-size:18pt} h2{font-size:14pt} h3{font-size:12pt;font-weight:bold}</style>
          </head><body>
          <h1>${title}</h1><p style='color:#888;font-size:10pt'>Subject: ${subject}</p><hr/>
          ${htmlLines}
          </body></html>`
