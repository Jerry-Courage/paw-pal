...
function StageMoment({ stage, node, answer, result, responding, continuing, onContinue, onAnswer, onRespond, onRefresh }: { stage: JourneyStage; node: WorldNode; answer: unknown; result?: EncounterAttemptResponse; responding: boolean; continuing: boolean; onContinue: () => void; onAnswer: (activity: EncounterActivity, value: unknown) => void; onRespond: (activity: EncounterActivity, value: unknown) => void; onRefresh: () => void }) {
  ...
  const mascotState = result?.correct === true ? 'celebrating' : result?.correct === false ? 'encouraging' : evidenceStage ? 'teaching' : stage.type === 'FLOW_INTRO' ? 'idle' : 'teaching'
  return <motion.section className="w-full" initial={{ opacity: 0, x: 24, scale: .985 }} animate={{ opacity: 1, x: 0, scale: 1 }} exit={{ opacity: 0, x: -24, scale: .985 }} transition={{ duration: .24 }} aria-label={`${stage.type.replaceAll('_', ' ')} learning stage`}>
    <div className="mb-4 flex items-center gap-3"><FlowCompanion state={mascotState} className="w-16 shrink-0 sm:w-20" /><div><p className="text-[10px] font-black uppercase tracking-[.2em] text-flow-violet">{stage.type.replaceAll('_', ' ')}</p><p className="mt-1 text-sm font-bold text-flow-muted">{evidenceStage ? 'Show Flow what clicked.' : stage.type === 'FLOW_INTRO' ? 'One focused moment at a time.' : 'Explore this, then continue when you are ready.'}</p></div></div>
    <div className="rounded-[1.5rem] border border-white/10 bg-[#141628] p-5 shadow-[0_22px_70px_rgba(0,0,0,.32)] sm:p-7">
      {stage.type === 'FLOW_INTRO' && <div className="py-4 text-center"><p className="text-[clamp(1.3rem,3vw,2rem)] font-black leading-tight">{normalizeReadableMath(stage.payload.text || stage.title)}</p><p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-flow-muted">Flow has shaped this checkpoint around your Source and current evidence.</p></div>}
    ...
    {!evidenceStage && <div className="mt-5 flex justify-end"><button onClick={onContinue} disabled={continuing} className="min-h-12 rounded-xl bg-flow-orange px-7 font-black text-flow-void shadow-[0_5px_0_#8f3600] transition active:translate-y-1 active:shadow-none disabled:opacity-50">{continuing ? 'Moving on…' : stage.optional ? 'Continue' : 'Continue'}</button></div>}
  </motion.section>
}
...
function ActivityCard({ activity, answer, result, historical, disabled, onAnswer, onSubmit }: { activity: EncounterActivity; answer: unknown; result?: EncounterAttemptResponse; historical: boolean; disabled: boolean; onAnswer: (value: unknown) => void; onSubmit: () => void }) {
  ...
  return <article className="border-y border-white/10 py-5 sm:py-6">{diagnostic && <div className="mb-5 flex items-start gap-3 border-l-4 border-flow-violet pl-4"><Target className="mt-0.5 h-5 w-5 shrink-0 text-flow-violet" /><div><p className="text-xs font-black uppercase tracking-[.16em] text-flow-violet">Quick diagnostic</p><p className="mt-1 text-sm text-flow-muted">This helps Flow choose the route. It won’t count against your mastery.</p></div></div>}{!remediation && !teaching && <p className="text-[clamp(1.125rem,1.8vw,1.5rem)] font-black leading-snug tracking-[-.01em] text-flow-ink">{normalizeReadableMath(activity.prompt)}</p>}{activity.instructions && <p className="mt-3 text-sm text-flow-muted">{activity.instructions}</p>}<SourceProvenance activity={activity} />
    {remediation ? <FocusedRemediationActivity activity={activity} /> : teaching ? <JourneyLearningObject object={activity} /> : activity.type === 'ordering' ? <OrderingActivity activity={activity} answer={answer} result={result} onAnswer={onAnswer} /> : activity.options?.length ? <ChoiceActivity activity={activity} answer={answer} result={result} onAnswer={onAnswer} /> : <ConversationalAnswer activity={activity} answer={answer} result={result} onAnswer={onAnswer} />}
    {!historical && !teaching && (result ? <div className={cn('mt-5 border-l-4 p-4 text-sm', result.correct === false ? 'border-rose-400 bg-rose-400/10' : 'border-flow-success bg-flow-success/10')}><p className="font-black text-flow-ink">{result.correct === false ? result.outcome === 'partial' ? 'Close — one distinction is missing.' : 'Not quite.' : result.correct === null ? 'Reflection captured.' : 'Exactly.'}</p><MarkdownContent>{result.feedback}</MarkdownContent>{result.correct === false && <button onClick={() => onAnswer(undefined)} className="mt-3 font-black text-flow-orange">Try again</button>}</div> : <button disabled={answer === undefined || !String(answer).trim() || disabled} onClick={onSubmit} className="mt-5 min-h-12 bg-flow-violet px-5 font-black text-flow-void disabled:opacity-40">{disabled ? 'Checking…' : 'Submit answer'}</button>)}
  </article>
}
