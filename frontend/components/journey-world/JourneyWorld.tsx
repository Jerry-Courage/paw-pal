...
import AskFlowPanel from '@/components/journey-world/AskFlowPanel'
import { useEffect, useMemo, useRef, useState } from 'react'
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query'
import { AnimatePresence, motion, useReducedMotion } from 'framer-motion'
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, BookOpen, Check, ChevronLeft, ChevronRight, Clock3, Coins, Crown, Flame, Headphones, Lightbulb, LockKeyhole, Menu, MessageCircle, Mic, Play, RefreshCw, RotateCw, Send, ShieldCheck, Swords, Target, Volume2, VolumeX, X } from 'lucide-react'
...
import JourneyLearningObject, { CollapsedPractice } from '@/components/journey-world/JourneyLearningObject'
import FlowReaction from '@/components/journey-world/FlowReaction'
import { EvidenceHighlightInteraction, MatchingInteraction, OrderingInteraction, RevealInteraction, SortingInteraction, TapTargetInteraction, canSubmitInteraction } from '@/components/journey-world/JourneyInteractions'
import { StepSolverCanvas } from '@/components/journey-world/TeachingCanvas'
...
import { beginJourneySubmission, journeySubmissionTransition, submissionErrorCopy } from '@/lib/journeyRuntime'
import type { EncounterActivity, EncounterAttemptResponse, JourneyPathDetail, JourneyRoadmapNode, JourneyRoadmapResponse, JourneyStage, JourneySubmission, RewardResponse, TeachingPlan, TeachingSessionResponse, TeachingTurn } from '@/types/journey'
...
export default function JourneyWorld({ pathId }: { pathId: string }) {
  ...
  const lessonOpenStarted = useRef(0)
  ...
  const mastery = pathQuery.data.mastery_state
  ...
  return <main className="flow-v2 relative min-h-[100dvh] overflow-x-hidden bg-flow-void text-flow-ink selection:bg-flow-orange selection:text-flow-void">
    ...
    <section className="relative mx-auto max-w-6xl px-3 pb-[max(6rem,env(safe-area-inset-bottom))] pt-8 sm:px-8">
      {mastery?.eligible && !mastery.passed && <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="mx-auto mb-8 max-w-4xl border-y border-flow-orange/40 bg-flow-orange/[.06] px-5 py-7 sm:px-8"><div className="grid gap-5 sm:grid-cols-[auto_1fr_auto] sm:items-center"><FlowCompanion state="encouraging" className="mx-auto w-20 sm:w-24" /><div><p className="flow-eyebrow">Learning path complete</p><h2 className="mt-2 text-2xl font-black sm:text-3xl">Now prove what you know.</h2><p className="mt-2 text-sm leading-6 text-flow-muted">A short mixed challenge checks understanding across this Journey. It cannot grant completion or rewards by itself.</p></div><Link href={`/learn/${pathId}/mastery`} className="flow-primary-button justify-center">{mastery.started ? 'Continue Mastery' : 'Start Mastery'} <ArrowRight className="h-4 w-4" /></Link></div></motion.section>}
      {mastery?.passed && <section className="mx-auto mb-8 max-w-4xl border-y border-flow-success/40 bg-flow-success/[.06] px-5 py-5"><p className="font-black text-flow-success">Mastery challenge cleared · {mastery.score}%</p></section>}
    ...
    {selected && !encounter && <NodeSheet node={selected} onClose={() => setSelected(null)} onStart={() => { if (selected.displayState !== 'locked') { lessonOpenStarted.current = performance.now(); setEncounter(selected); setSelected(null) } }} />}
    <AnimatePresence>{encounter && <StudyEncounter node={encounter} clickStarted={lessonOpenStarted.current} onClose={closeEncounter} onCompleted={async (result, mastery) => { closeEncounter(); setReward(result); setRewardMastery(mastery); await refresh() }} />}</AnimatePresence>
...
function StudyEncounter({ node, clickStarted, onClose, onCompleted }: { node: WorldNode; clickStarted: number; onClose: () => void; onCompleted: (reward: RewardResponse, mastery: number) => void }) {
  ...
  const [submissionNotice, setSubmissionNotice] = useState<JourneySubmission | null>(null)
  ...
  const sessionQuery = useQuery({ queryKey: ['teaching-session', node.id], queryFn: async () => {
    requestStarted.current = performance.now()
    const response = await learningApi.getTeachingSession(node.id)
    console.info('[JOURNEY PERF]', { operation: 'lesson_open_frontend_request', concept_id: node.id, network_ms: Math.round(performance.now() - requestStarted.current) })
    return response.data as TeachingSessionResponse
  } })
  ...
  const respond = useMutation({
    mutationFn: ({ activity, response, idempotencyKey }: { activity: EncounterActivity; response: unknown; idempotencyKey: string }) => learningApi.submitTeachingResponse(node.id, { activity_id: activity.id, response, idempotency_key: idempotencyKey, objective_id: activity.objective_id, moment_id: activity.tutor?.moment_id, plan_revision: session?.teaching_plan?.plan_revision }).then(reply => reply.data as TeachingSessionResponse),
    onMutate: () => setSubmissionError(''),
    onSuccess: (data, variables) => {
      const transition = journeySubmissionTransition(data)
      queryClient.setQueryData(['teaching-session', node.id], data)
      const evaluation = data.evaluation
      setSubmissionNotice(transition.submission)
    ...
    onError: (caught) => {
      const error = caught as AxiosError<{ error?: string; message?: string; recoverable?: boolean; next_action?: string }>
      setSubmissionError(submissionErrorCopy(error.response?.status, error.response?.data))
    },
    onSettled: () => {
      if (submissionStarted.current) console.info('[JOURNEY PERF]', { operation: 'check_submission_frontend_update', concept_id: node.id, total_ms: Math.round(performance.now() - submissionStarted.current) })
      submissionStarted.current = 0
  ...
  useEffect(() => {
    if (!session?.active_activity_id) return
    setAnswers(current => session.active_activity_id in current ? current : { ...current, [session.active_activity_id]: '' })
  }, [session?.active_activity_id])
  useEffect(() => {
    if (!session || !clickStarted) return
    console.info('[JOURNEY PERF]', { operation: 'lesson_open_frontend_render', concept_id: node.id, total_ms: Math.round(performance.now() - clickStarted) })
  }, [session, clickStarted, node.id])
  useEffect(() => {
    ...
    if (previousSatisfiedRef.current !== null && satisfied > previousSatisfiedRef.current) {
      ...
      sounds.play('objective_complete', `objective-${newlySatisfied?.id || satisfied}`)
  ...
  if (sessionQuery.isLoading) return <PreparingEncounter onClose={onClose} />
  ...
  const activeStage = session.player?.active_stage
  ...
  const quickActions = latestTurn?.payload.quick_replies?.length
    ? latestTurn.payload.quick_replies.map(label => [label, label] as const)
    : session.teaching_phase === 'REMEDIATE'
      ? [['Try again', "Okay, let's try again"], ['One more example', 'Show me another example']] as const
      : session.teaching_phase === 'ADVANCE'
        ? [['Keep going', 'Continue'], ['Quick recap', 'Give me a quick recap']] as const
        : [['Show me an example', 'Show me an example'], ['Test me', 'Test me on this checkpoint'], ['I have a question', 'I have a question']] as const
  const submitActivity = (activity: EncounterActivity, value: unknown) => {
    if (!beginJourneySubmission(submissionLock)) return
    submissionStarted.current = performance.now()
    respond.mutate({ activity, response: activity.options ? { choice: value } : activity.type === 'ordering' ? { order: value } : { text: value }, idempotencyKey: crypto.randomUUID() })
  }
  return <motion.div className="flow-v2 flow-atmosphere fixed inset-0 z-[60] flex min-w-0 flex-col overflow-hidden text-flow-ink" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><header className="z-20 shrink-0 bg-flow-void/80 px-3 pt-[max(.45rem,env(safe-area-inset-top))] backdrop-blur sm:px-6"><div className="mx-auto grid max-w-[960px] grid-cols-[2.75rem_1fr_5.5rem] items-center gap-3"><button onClick={onClose} aria-label="Back to Journey" className="grid h-11 w-11 place-items-center text-flow-muted hover:text-flow-ink focus-visible:ring-2 focus-visible:ring-flow-orange"><ArrowLeft className="h-5 w-5" /></button><button onClick={() => setProgressOpen(true)} className="flex min-h-11 items-center justify-center gap-2" aria-label={`Open progress. ${satisfiedCount} of ${objectiveState.length} ideas complete`}>{objectiveState.map((objective, index) => <motion.span layout key={objective.id} className={cn('h-2 w-full max-w-12 rounded-full', objective.satisfied ? 'bg-flow-success' : index === currentObjectiveIndex ? 'bg-flow-orange' : 'bg-white/10')} />)}</button><div className="flex justify-end"><button onClick={() => sounds.setMuted(!sounds.muted)} aria-label={sounds.muted ? 'Turn sound on' : 'Mute sound'} className="grid h-11 w-11 place-items-center text-flow-muted hover:text-flow-ink">{sounds.muted ? <VolumeX className="h-5 w-5" /> : <Volume2 className="h-5 w-5" />}</button><button onClick={() => setComposerOpen(value => !value)} aria-label="Ask Flow" className="grid h-11 w-11 place-items-center text-flow-violet"><MessageCircle className="h-5 w-5" /></button></div></div></header>
    <main className="min-h-0 flex-1 overflow-y-auto overscroll-y-contain px-4 py-3 touch-pan-y sm:px-6 sm:py-5"><div className="mx-auto flex min-h-full w-full max-w-[960px] flex-col"><AnimatePresence>{checkpointNotice && <motion.div role="status" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="mx-auto mb-2 flex items-center gap-2 rounded-full bg-flow-success/10 px-4 py-2"><Check className="h-4 w-4 text-flow-success" /><p className="text-sm font-black">Nice. That idea is yours.</p></motion.div>}</AnimatePresence>{submissionNotice?.feedback && <div role="status" className={cn('mx-auto mb-3 w-full max-w-2xl border-l-4 px-4 py-3 text-sm', submissionNotice.correct ? 'border-flow-success bg-flow-success/10' : submissionNotice.outcome === 'ungradable_system_error' ? 'border-rose-300 bg-rose-300/10' : 'border-flow-orange bg-flow-orange/10')}><p className="font-black text-flow-ink">{submissionNotice.correct ? 'That works.' : submissionNotice.outcome === 'ungradable_system_error' ? 'Your answer is safe.' : submissionNotice.outcome === 'learning_signal' ? 'Let’s build it together.' : submissionNotice.outcome === 'partial' ? 'Nearly. One piece is missing.' : 'Here’s the missing piece.'}</p><p className="mt-1 leading-6 text-flow-muted">{submissionNotice.feedback}</p></div>}{submissionError && <div role="alert" className="mx-auto mb-3 w-full max-w-2xl border-l-4 border-rose-300 bg-rose-300/10 px-4 py-3 text-sm"><p className="font-black text-flow-ink">Your place is safe.</p><p className="mt-1 text-flow-muted">{submissionError}</p><div className="mt-2 flex gap-4">{respond.variables && <button className="min-h-10 font-black text-flow-orange" onClick={() => { if (beginJourneySubmission(submissionLock)) { submissionStarted.current = performance.now(); respond.mutate(respond.variables!) } }}>Try again</button>}<button className="min-h-10 font-black text-flow-violet" onClick={() => sessionQuery.refetch()}>Refresh saved lesson</button></div></div>}<div className="flex flex-1 items-center"><AnimatePresence mode="wait">{activeStage && <StageMoment key={activeStage.id} stage={activeStage} node={node} plan={session.teaching_plan} answer={activeStage.payload.activity ? answers[activeStage.payload.activity.id] : undefined} result={activeStage.payload.activity ? results[activeStage.payload.activity.id] : undefined} responding={respond.isPending} continuing={continueStage.isPending} onContinue={() => continueStage.mutate(activeStage.id)} onAnswer={(activity, value) => { setSubmissionNotice(null); setSubmissionError(''); setAnswers(current => ({ ...current, [activity.id]: value })); setResults(current => { const next = { ...current }; delete next[activity.id]; return next }) }} onRespond={submitActivity} onRefresh={() => sessionQuery.refetch()} />}</AnimatePresence></div>{send.isPending && <div className="mx-auto mt-3 flex items-center gap-3"><FlowCompanion state="thinking" className="w-10" /><p className="text-sm font-bold text-flow-muted">Thinking it through…</p></div>}{send.isError && <p role="alert" className="mt-3 text-center text-sm text-rose-300">That didn’t reach Flow. Your place is safe.</p>}{session.completion_evaluation.complete && <motion.section initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} className="mx-auto mt-4 text-center"><span className="mx-auto grid h-11 w-11 place-items-center rounded-full bg-flow-success text-flow-void"><Check /></span><p className="mt-3 text-xl font-black">You’ve got this one.</p><button onClick={() => complete.mutate()} disabled={complete.isPending} className="mt-4 min-h-12 rounded-full bg-flow-success px-6 font-black text-flow-void">{complete.isPending ? 'Saving your win…' : 'Keep moving'}</button></motion.section>}<div ref={endRef} /></div></main>
    {composerOpen && <AskFlowPanel nodeId={node.id} onClose={() => setComposerOpen(false)} onVoice={() => { setComposerOpen(false); setVoiceOpen(true) }} onCards={() => { setComposerOpen(false); send.mutate('make flashcards') }} />}<AnimatePresence>{progressOpen && <motion.div className="fixed inset-0 z-[65] flex items-end bg-black/60 sm:items-center sm:justify-center sm:p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setProgressOpen(false)}><motion.section role="dialog" aria-modal="true" aria-labelledby="journey-progress-title" initial={{ y: 40 }} animate={{ y: 0 }} exit={{ y: 40 }} onClick={event => event.stopPropagation()} className="w-full max-w-lg border-t border-white/10 bg-flow-raised p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] shadow-2xl sm:rounded-3xl sm:border"><div className="flex items-center justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-flow-orange">Your route</p><h2 id="journey-progress-title" className="mt-1 text-xl font-black">{satisfiedCount} of {objectiveState.length} ideas</h2></div><button onClick={() => setProgressOpen(false)} aria-label="Close Journey progress" className="grid h-11 w-11 place-items-center text-flow-muted"><X /></button></div><ol className="mt-5 space-y-4">{objectiveState.map((objective, index) => <li key={objective.id} className="flex items-start gap-3"><span className={cn('mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-full text-xs font-black', objective.satisfied ? 'bg-flow-success text-flow-void' : index === currentObjectiveIndex ? 'bg-flow-orange text-flow-void' : 'bg-white/10 text-flow-muted')}>{objective.satisfied ? <Check className="h-4 w-4" /> : index + 1}</span><p className={cn('text-sm font-bold', index === currentObjectiveIndex || objective.satisfied ? 'text-flow-ink' : 'text-flow-muted')}>{normalizeReadableMath(conciseCheckpointTitle(objective.text))}</p></li>)}</ol></motion.section></motion.div>}</AnimatePresence><AnimatePresence>{voiceOpen && <TeachingVoiceMode session={session} onClose={closeVoice} />}</AnimatePresence>{session.completion_evaluation.normal_requirements_met && !session.completion_evaluation.feynman.passed && <FeynmanFinale node={node} session={session} onRefresh={() => sessionQuery.refetch()} />}</motion.div>
}
...
function StageMoment({ stage, node, plan, answer, result, responding, continuing, onContinue, onAnswer, onRespond, onRefresh }: { stage: JourneyStage; node: WorldNode; plan?: TeachingPlan | null; answer: unknown; result?: EncounterAttemptResponse; responding: boolean; continuing: boolean; onContinue: () => void; onAnswer: (activity: EncounterActivity, value: unknown) => void; onRespond: (activity: EncounterActivity, value: unknown) => void; onRefresh: () => void }) {
  const activity = stage.payload.activity
  const reduced = useReducedMotion()
  const evidenceStage = Boolean(stage.activity_id)
  const [revealed, setRevealed] = useState(activity?.content?.revealed_steps || 1)
  const [revealError, setRevealError] = useState(false)
  const reveal = async (count: number) => { try { await learningApi.revealTeachingStep(node.id, activity!.id, count); setRevealed(count); setRevealError(false) } catch { setRevealError(true) } }
  useEffect(() => {
    if (process.env.NODE_ENV !== 'development') return
    console.debug('[Journey canvas]', { objectiveId: activity?.objective_id || plan?.objective_id, planOrigin: plan?.origin || 'unknown', subjectFamily: plan?.subject_family || activity?.content?.subject_family, representationRequested: plan?.recommended_representation, representationRendered: activity?.content?.knowledge_type || activity?.type || stage.type, interaction: activity?.type || 'none', fallbackReason: plan?.origin === 'fallback' ? 'AI disabled, rejected, or unavailable; inspect backend Journey TeachingPlan log' : null })
  }, [activity, plan, stage.type])
  const reaction = result?.correct === true ? 'CORRECT' : result?.correct === false ? 'ENCOURAGING' : stage.type === 'FLOW_INTRO' ? 'WELCOME' : evidenceStage ? 'CURIOUS' : 'TEACHING'
  const line = activity?.dialogue !== undefined ? (result?.feedback || activity.dialogue) : result?.correct === true ? 'Yep. That clicks.' : result?.correct === false ? 'Close. Watch what changes.' : stage.type === 'FLOW_INTRO' ? 'Ready? One idea at a time.' : evidenceStage ? 'Your turn.' : activity?.type === 'process' ? 'Watch what happens inside.' : activity?.type === 'comparison' ? 'Keep these two apart.' : activity?.type === 'worked_example' ? 'Let’s work it through.' : 'Look here.'
  const action = stage.type === 'FLOW_INTRO' ? 'Start' : activity?.type === 'worked_example' ? 'Next' : activity?.type === 'process' || activity?.type === 'sequence' ? 'Show me' : 'Continue'
  return <motion.section className="w-full py-2" initial={reduced ? false : { opacity: 0, x: 28, scale: .98 }} animate={{ opacity: 1, x: 0, scale: 1 }} exit={reduced ? { opacity: 0 } : { opacity: 0, x: -28, scale: .98 }} transition={{ duration: .3, ease: 'easeOut' }} aria-label="Current learning moment">
    <FlowReaction state={reaction} line={line} position={activity?.mascot_position || (evidenceStage ? 'edge' : activity?.type === 'worked_example' ? 'beside' : 'upper')} className="mb-2" />
    <div className="mx-auto w-full max-w-[900px] px-0 sm:px-5">
      {stage.type === 'FLOW_INTRO' && <div className="py-5 text-center"><p className="mx-auto max-w-xl text-[clamp(1.35rem,3vw,2.35rem)] font-black leading-tight tracking-[-.04em]">{normalizeReadableMath(stage.payload.text || stage.title)}</p></div>}
      {activity?.type === 'worked_example' && activity.tutor ? <JourneyLearningObject object={activity} shown={revealed} onReveal={reveal} /> : activity && <ActivityCard activity={activity} answer={answer} result={result} historical={false} disabled={responding} onAnswer={value => onAnswer(activity, value)} onSubmit={() => onRespond(activity, answer)} />}
      {stage.type === 'VIDEO' && stage.payload.video && <VideoLearningObject nodeId={node.id} videos={[stage.payload.video]} />}
      {stage.type === 'FLASHCARD' && stage.payload.cards && <FlashcardLearningObject nodeId={node.id} cards={stage.payload.cards} onSaved={onRefresh} />}
      {stage.type === 'PODCAST' && stage.payload.podcast && <PodcastLearningObject nodeId={node.id} podcast={stage.payload.podcast} />}
    </div>
    {revealError && <p role="alert" className="text-center text-rose-300">Could not save that step. Please try again.</p>}
    {!evidenceStage && <div className="mx-auto mt-5 flex max-w-[800px] justify-center"><button onClick={onContinue} disabled={continuing || Boolean(activity?.tutor && activity.type === 'worked_example' && revealed < (activity.content?.steps?.length || 0))} className="min-h-12 rounded-full bg-flow-orange px-7 font-black text-flow-void shadow-[0_4px_0_#8f3600] transition hover:-translate-y-0.5 active:translate-y-1 active:shadow-none disabled:opacity-50">{continuing ? 'One sec…' : action}</button></div>}
  </motion.section>
}

function PodcastLearningObject({ nodeId, podcast }: { nodeId: string; podcast: NonNullable<JourneyStage['payload']['podcast']> }) {
  const save = useMutation({ mutationFn: () => learningApi.saveTeachingArtifact(nodeId, { type: 'podcast', title: podcast.title || 'Journey audio', content: podcast, external_object_id: podcast.audio_url }) })
  return <section className="py-3"><div className="flex items-center gap-3"><span className="grid h-12 w-12 place-items-center rounded-full bg-flow-violet/15 text-flow-violet"><Headphones /></span><div><p className="font-black">{podcast.title || 'Quick audio breakdown'}</p><p className="text-xs text-flow-muted">Optional listening · never counted as mastery evidence</p></div></div><audio controls preload="metadata" className="mt-5 w-full" src={podcast.audio_url}>Your browser does not support audio playback.</audio><div className="mt-4 flex items-center justify-between gap-3"><p className="text-xs text-flow-muted">{podcast.provenance || 'From this Journey checkpoint'}</p><button onClick={() => save.mutate()} disabled={save.isPending || save.isSuccess} className="min-h-11 text-sm font-black text-flow-violet">{save.isSuccess ? 'Saved' : save.isPending ? 'Saving…' : 'Save audio'}</button></div></section>
}

function ConversationTurn({ turn, node, currentActivityId, answer, result, responding, onAnswer, onRespond, onRefresh }: { turn: TeachingTurn; node: WorldNode; currentActivityId: string; answer: unknown; result?: EncounterAttemptResponse; responding: boolean; onAnswer: (activity: EncounterActivity, value: unknown) => void; onRespond: (activity: EncounterActivity, value: unknown) => void; onRefresh: () => void }) {
  const activity = turn.payload.activity
  if (turn.role === 'learner') return <div className="ml-auto w-fit max-w-[85%] break-words rounded-[1.2rem_1.2rem_.25rem_1.2rem] bg-flow-orange px-3.5 py-2.5 text-flow-void shadow-[0_3px_0_#8f3600] sm:max-w-[63%] sm:px-4"><p className="text-[15px] font-bold leading-relaxed">{turn.content || 'Answer submitted'}</p></div>
  ...
  return <section className="relative pl-11 sm:pl-14"><FlowCompanion state={avatarState} className="absolute left-0 top-0 w-9 sm:w-10" label="Flow" /><div className="max-w-[760px]"><p className="mb-1 text-[10px] font-black uppercase tracking-[.16em] text-flow-violet">Flow</p><MarkdownContent>{turn.content}</MarkdownContent>{turn.payload.learning_objects?.map(object => <div key={object.id} className="mt-6 border-y border-white/10 py-5 sm:py-6"><JourneyLearningObject object={object} /></div>)}{activity && <div className="mt-6"><ActivityCard activity={activity} answer={answer} result={result} historical={['check', 'apply', 'transfer', 'diagnose'].includes(activity.purpose) && activity.id !== currentActivityId} disabled={responding} onAnswer={value => onAnswer(activity, value)} onSubmit={() => onRespond(activity, answer)} /></div>}{turn.kind === 'video' && <VideoLearningObject videos={turn.payload.videos || []} />}{turn.kind === 'flashcards' && <FlashcardLearningObject nodeId={node.id} cards={turn.payload.cards || []} onSaved={onRefresh} />}</div></section>
}

function VideoLearningObject({ videos, nodeId }: { videos: NonNullable<TeachingTurn['payload']['videos']>; nodeId?: string }) { const [active, setActive] = useState(0); const video = videos[active]; const save = useMutation({ mutationFn: () => learningApi.saveTeachingArtifact(nodeId!, { type: 'video_reference', title: video.title, content: video, external_object_id: video.video_id }) }); if (!video) return null; return <figure className="mt-5 overflow-hidden border border-white/10 bg-flow-raised"><div className="aspect-video bg-black"><iframe className="h-full w-full" src={video.embed_url || `https://www.youtube-nocookie.com/embed/${video.video_id}?rel=0`} title={video.title} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen /></div><figcaption className="p-4 sm:p-5"><p className="text-base font-black text-flow-ink"><Play className="mr-2 inline h-4 w-4 text-flow-violet" />{video.title}</p><p className="mt-1 text-xs text-flow-muted">{video.channel}{video.duration_str ? ` · ${video.duration_str}` : ''}</p>{video.why && <p className="mt-3 text-sm leading-6 text-flow-muted">{video.why}</p>}<p className="mt-4 text-sm font-bold text-flow-ink">When you’re done, tell Flow what became clearer. Watching alone won’t count as understanding.</p><div className="mt-3 flex flex-wrap items-center gap-4">{videos.length > 1 && <button onClick={() => setActive(index => (index + 1) % videos.length)} className="text-xs font-black text-flow-violet">Try another video</button>}{nodeId && <button onClick={() => save.mutate()} disabled={save.isPending || save.isSuccess} className="text-xs font-black text-flow-violet">{save.isSuccess ? 'Saved' : 'Save video'}</button>}<a href={video.url} target="_blank" rel="noreferrer" className="text-xs font-bold text-flow-muted underline">Open externally if playback is blocked</a></div></figcaption></figure> }
function FlashcardLearningObject({ nodeId, cards, onSaved }: { nodeId: string; cards: NonNullable<TeachingTurn['payload']['cards']>; onSaved: () => void }) { const sounds=useFlowSound(); const [saved,setSaved]=useState(false); const [index,setIndex]=useState(0); const [flipped,setFlipped]=useState(false); const card=cards[index]; const save=useMutation({mutationFn:()=>learningApi.saveTeachingFlashcards(nodeId,cards),onSuccess:()=>{setSaved(true);sounds.play('flashcard_save',`deck-${nodeId}`);onSaved()}}); if(!card)return null; const flip=()=>{setFlipped(v=>!v);sounds.play('flashcard_flip')}; const move=(direction:number)=>{setIndex(v=>(v+direction+cards.length)%cards.length);setFlipped(false)}; return <section className="mx-auto mt-5 max-w-xl [perspective:1000px]" aria-label="Flow flashcards" onKeyDown={event=>{if(event.key==='ArrowRight')move(1);if(event.key==='ArrowLeft')move(-1);if(event.key===' '||event.key==='Enter'){event.preventDefault();flip()}}} tabIndex={0}><div className="mb-3 flex items-center justify-between text-xs font-black text-flow-muted"><span>Remember this</span><span>{index+1} / {cards.length}</span></div><button onClick={flip} aria-pressed={flipped} className="relative min-h-72 w-full rounded-[2rem] bg-gradient-to-br from-flow-violet/20 to-flow-orange/10 p-7 text-center shadow-[0_20px_60px_rgba(0,0,0,.28)] focus-visible:ring-2 focus-visible:ring-flow-orange"><motion.div key={`${index}-${flipped}`} initial={{rotateY:90,opacity:0}} animate={{rotateY:0,opacity:1}} transition={{duration:.36}}><p className="mt-8 text-[clamp(1.25rem,3vw,1.8rem)] font-black leading-snug">{flipped?card.answer:card.question}</p><p className="mt-10 flex items-center justify-center gap-2 text-xs font-bold text-flow-muted"><RotateCw className="h-4 w-4"/>Tap or press Space to flip</p></motion.div></button><div className="mt-3 flex items-center justify-between"><button onClick={()=>move(-1)} aria-label="Previous flashcard" className="grid h-11 w-11 place-items-center"><ChevronLeft/></button><button onClick={()=>save.mutate()} disabled={saved||save.isPending} className="min-h-11 text-sm font-black text-flow-violet">{saved?'Saved ✓':save.isPending?'Saving…':'Save cards'}</button><button onClick={()=>move(1)} aria-label="Next flashcard" className="grid h-11 w-11 place-items-center"><ChevronRight/></button></div></section>}

function ActivityCard({ activity, answer, result, historical, disabled, onAnswer, onSubmit }: { activity: EncounterActivity; answer: unknown; result?: EncounterAttemptResponse; historical: boolean; disabled: boolean; onAnswer: (value: unknown) => void; onSubmit: () => void }) {
  const teaching = activity.purpose === 'learn' || ['comparison', 'worked_example', 'concept', 'key_idea', 'process', 'sequence', 'cause_effect', 'formula', 'example', 'diagram', 'callout'].includes(activity.type)
  const diagnostic = activity.purpose === 'diagnose'
  const remediation = activity.purpose === 'remediate'
  if (historical) return <CollapsedPractice activity={activity} />
  return <article className="py-2">{diagnostic && <div className="mx-auto mb-4 flex max-w-2xl items-start gap-3 rounded-2xl bg-flow-violet/10 p-4"><Target className="mt-0.5 h-5 w-5 shrink-0 text-flow-violet" /><div><p className="font-black text-flow-violet">Quick warm-up</p><p className="mt-1 text-sm text-flow-muted">This only helps Flow choose where to begin.</p></div></div>}{!remediation && !teaching && <p className="mx-auto max-w-2xl text-[clamp(1.2rem,2vw,1.65rem)] font-black leading-snug tracking-[-.02em] text-flow-ink">{normalizeReadableMath(activity.prompt)}</p>}{activity.instructions && <p className="mx-auto mt-3 max-w-2xl text-sm text-flow-muted">{activity.instructions}</p>}
    {remediation || teaching ? <JourneyLearningObject object={activity} /> : activity.type === 'ordering' ? <OrderingInteraction activity={activity} answer={answer} result={result} onAnswer={onAnswer} /> : activity.type === 'matching' ? <MatchingInteraction activity={activity} answer={answer} result={result} onAnswer={onAnswer} /> : activity.type === 'sorting' ? <SortingInteraction activity={activity} answer={answer} result={result} onAnswer={onAnswer} /> : activity.type === 'tap_target' ? <TapTargetInteraction activity={activity} answer={answer} result={result} onAnswer={onAnswer} /> : activity.type === 'evidence_highlight' ? <EvidenceHighlightInteraction activity={activity} answer={answer} result={result} onAnswer={onAnswer} /> : activity.type === 'step_solver' ? <StepSolverCanvas object={activity} onAnswer={onAnswer} /> : activity.type === 'reveal' ? <RevealInteraction activity={activity} /> : activity.options?.length ? <ChoiceActivity activity={activity} answer={answer} result={result} onAnswer={onAnswer} /> : <ConversationalAnswer activity={activity} answer={answer} result={result} onAnswer={onAnswer} />}
    {!historical && !teaching && activity.type !== 'reveal' && (result ? <motion.div initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} className={cn('mx-auto mt-5 max-w-2xl border-l-2 py-1 pl-4 text-sm', result.correct === false ? 'border-rose-300' : 'border-flow-success')}><p className="font-black text-flow-ink">{result.correct === false ? result.outcome === 'partial' ? 'Nearly. One piece is missing.' : 'Look at this part.' : result.correct === null ? 'Answer received.' : 'Exactly.'}</p><MarkdownContent>{result.feedback}</MarkdownContent>{result.correct === false && <button onClick={() => onAnswer(undefined)} className="mt-2 min-h-10 font-black text-flow-orange">Try again</button>}</motion.div> : <div className="mt-5 flex justify-center"><button disabled={!canSubmitInteraction(activity, answer) || disabled} onClick={onSubmit} className="min-h-12 rounded-full bg-flow-violet px-7 font-black text-flow-void shadow-[0_4px_0_#5140a8] transition active:translate-y-1 active:shadow-none disabled:opacity-40">{disabled ? 'Checking…' : activity.type === 'ordering' ? 'Check order' : 'Check'}</button></div>)}
  </article>
}

function FocusedRemediationActivity({ activity }: { activity: EncounterActivity }) {
  const content = activity.content
  return <section className="border-l-4 border-flow-orange py-1 pl-4 sm:pl-5"><p className="text-[10px] font-black uppercase tracking-[.18em] text-flow-orange">Different angle</p><h3 className="mt-2 text-xl font-black text-flow-ink">{content?.title || activity.prompt}</h3>{content?.lead && <p className="mt-2 text-sm leading-6 text-flow-muted">{content.lead}</p>}{content?.columns && content.rows ? <ComparisonActivity activity={activity} /> : content?.steps?.length ? <ol className="mt-5 space-y-3">{content.steps.map((step, index) => <li key={index} className="flex gap-3 text-sm leading-6 text-flow-muted"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-flow-orange/15 font-black text-flow-orange">{index + 1}</span><span>{normalizeReadableMath(typeof step === 'string' ? step : step.body)}</span></li>)}</ol> : <MarkdownContent>{content?.example || ''}</MarkdownContent>}{content?.takeaway && <div className="mt-5 border-t border-white/10 pt-4"><p className="text-[10px] font-black uppercase tracking-[.16em] text-flow-violet">Keep this distinction</p><MarkdownContent>{content.takeaway}</MarkdownContent></div>}</section>
}
...
function ChoiceActivity({ activity, answer, result, onAnswer }: { activity: EncounterActivity; answer: unknown; result?: EncounterAttemptResponse; onAnswer: (value: unknown) => void }) {
  ...
  return <div className={cn('mt-5 grid gap-2', prediction && 'sm:grid-cols-3')} role="radiogroup" aria-label={prediction ? 'Make your prediction' : 'Choose one answer'}>{activity.options?.map((option, optionIndex) => <button key={optionIndex} role="radio" disabled={Boolean(result)} onClick={() => onAnswer(optionIndex)} aria-checked={answer === optionIndex} className={cn('relative min-h-12 rounded-xl border px-4 py-3 text-left text-sm font-bold transition focus-visible:ring-2 focus-visible:ring-flow-orange', prediction && 'min-h-20 text-center', answer === optionIndex ? 'border-flow-orange bg-flow-orange/15 text-flow-ink' : 'border-white/15 hover:border-flow-violet/70', result && answer === optionIndex && result.correct === false && 'border-rose-400 bg-rose-400/10')}><span className={cn('mb-1 block text-[10px] font-black uppercase tracking-[.16em] text-flow-muted', !prediction && 'sr-only')}>Decision {optionIndex + 1}</span>{normalizeReadableMath(option)}</button>)}</div>
}
...
function ConversationalAnswer({ activity, answer, result, onAnswer }: { activity: EncounterActivity; answer: unknown; result?: EncounterAttemptResponse; onAnswer: (value: unknown) => void }) {
  ...
  const fieldId = `activity-answer-${activity.id}`
  return <div className="mt-6 border-l-2 border-flow-violet/60 pl-4"><label htmlFor={fieldId} className="mb-2 block text-sm font-black text-flow-ink">{reflection ? 'Teach it back to Flow' : 'Your answer'}</label><p className="text-xs leading-5 text-flow-muted">{reflection ? 'Explain it as if you were talking it through with Flow.' : 'One clear thought is enough. This is separate from the Journey chat below.'}</p><textarea id={fieldId} value={String(answer ?? '')} disabled={Boolean(result)} onChange={event => { event.currentTarget.style.height = 'auto'; event.currentTarget.style.height = `${Math.min(event.currentTarget.scrollHeight, 180)}px`; onAnswer(event.target.value) }} rows={3} placeholder={reflection ? 'I would explain it like this…' : 'My answer is…'} className="mt-2 min-h-24 w-full resize-none overflow-y-auto border border-white/15 bg-white/[.025] px-3 py-3 text-base leading-relaxed text-flow-ink outline-none placeholder:text-flow-muted/60 focus:border-flow-orange" /></div>
}
...
function PreparingEncounter({ onClose }: { onClose: () => void }) { return <main className="flow-v2 fixed inset-0 z-[60] grid min-h-[100dvh] place-items-center bg-flow-void p-6 text-center text-flow-ink"><button onClick={onClose} aria-label="Back to Journey" className="absolute left-4 top-[max(1rem,env(safe-area-inset-top))] grid h-11 w-11 place-items-center text-flow-muted"><ArrowLeft /></button><div><FlowCompanion state="thinking" className="mx-auto w-36" /><p className="text-xs font-black uppercase tracking-[.2em] text-flow-orange">Preparing this lesson</p><h1 className="mt-2 text-2xl font-black">Flow is lining up the first idea…</h1><p className="mt-2 text-sm text-flow-muted">Your Journey position is already safe.</p></div></main> }
