...
function StudyEncounter({ node, onClose, onCompleted }: { node: WorldNode; onClose: () => void; onCompleted: (reward: RewardResponse, mastery: number) => void }) {
  ...
  return <motion.div className="flow-v2 flow-atmosphere fixed inset-0 z-[60] flex min-w-0 flex-col overflow-hidden text-flow-ink" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><header className="z-20 flex shrink-0 items-center gap-2 border-b border-white/[.07] bg-flow-void/95 px-2 pb-2 pt-[max(.5rem,env(safe-area-inset-top))] backdrop-blur sm:gap-3 sm:px-6 sm:py-3"><button onClick={onClose} className="inline-flex min-h-11 shrink-0 items-center gap-2 text-sm font-black text-flow-muted"><ArrowLeft className="h-5 w-5" /><span className="hidden sm:inline">Journey</span></button><h1 className="min-w-0 flex-1 truncate text-xs font-black min-[390px]:text-sm sm:text-lg">{normalizeReadableMath(node.title)}</h1><span className="shrink-0 text-xs font-black text-flow-muted">{progress}%</span></header><div className="h-1 shrink-0 bg-white/[.07]"><motion.div className="h-full bg-gradient-to-r from-flow-orange to-flow-violet" animate={{ width: `${progress}%` }} /></div>
...
function ConversationTurn({ turn, node, answer, result, responding, onAnswer, onRespond, onRefresh }: { turn: TeachingTurn; node: WorldNode; answer: unknown; result?: EncounterAttemptResponse; responding: boolean; onAnswer: (activity: EncounterActivity, value: unknown) => void; onRespond: (activity: EncounterActivity, value: unknown) => void; onRefresh: () => void }) {
  ...
  if (turn.role === 'learner') return <div className="ml-auto max-w-[90%] break-words border-r-4 border-flow-orange pr-3 text-right sm:max-w-[82%] sm:pr-4"><p className="text-base font-bold">{turn.content || 'Response submitted'}</p></div>
