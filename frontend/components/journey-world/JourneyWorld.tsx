...
import { ArrowDown, ArrowLeft, ArrowUp, BookOpen, Check, ChevronRight, Clock3, Coins, Crown, Flame, Lightbulb, LockKeyhole, Menu, MessageCircle, Mic, Play, Plus, RefreshCw, Send, ShieldCheck, Swords, Target, X } from 'lucide-react'
...
function StudyEncounter({ node, onClose, onCompleted }: { node: WorldNode; onClose: () => void; onCompleted: (reward: RewardResponse, mastery: number) => void }) {
  ...
  const respond = useMutation({ mutationFn: ({ activity, response }: { activity: EncounterActivity; response: unknown }) => learningApi.submitTeachingResponse(node.id, { activity_id: activity.id, response }).then(reply => reply.data as TeachingSessionResponse), onSuccess: (data, variables) => { queryClient.setQueryData(['teaching-session', node.id], data); if (data.evaluation) setResults(current => ({ ...current, [variables.activity.id]: { attempt_id: data.evaluation!.attempt_id, correct: data.evaluation!.correct, score: data.evaluation!.score, feedback: data.evaluation!.feedback, explanation: '', hint: '', evidence_score: data.mastery, attempt_number: 1, recommend_flow: data.evaluation!.correct === false } })) } })
  const complete = useMutation({ mutationFn: () => node.status === 'completed' ? learningApi.reviewConcept(node.id, session?.mastery || 0) : learningApi.completeConcept(node.id, session?.mastery || 0), onSuccess: response => onCompleted(response.data.reward || { xp: 0, flowcoins: 0, level: { previous: 0, current: 0, leveled_up: false }, streak: { current: 0, increased: false }, missions: [], achievements: [] }, response.data.mastery ?? session?.mastery ?? 0) })
  ...
  return <motion.div className="fixed inset-0 z-[60] flex flex-col bg-flow-void text-flow-ink" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><header className="flow-v2 z-20 flex shrink-0 items-center gap-3 border-b border-white/10 bg-flow-void/95 px-3 py-3 backdrop-blur sm:px-6"><button onClick={onClose} className="inline-flex min-h-11 items-center gap-2 text-sm font-black text-flow-muted"><ArrowLeft className="h-5 w-5" /><span className="hidden sm:inline">Journey</span></button><h1 className="min-w-0 flex-1 truncate text-sm font-black sm:text-lg">{normalizeReadableMath(node.title)}</h1><span className="text-xs font-black text-flow-muted">{progress}%</span></header><div className="h-1 shrink-0 bg-white/10"><motion.div className="h-full bg-flow-orange" animate={{ width: `${progress}%` }} /></div>
    <main className="flow-v2 min-h-0 flex-1 overflow-y-auto overscroll-y-contain px-4 py-8 touch-pan-y"><div className="mx-auto max-w-3xl space-y-8">{session.turns.map(turn => <ConversationTurn key={turn.id} turn={turn} node={node} answer={turn.payload.activity ? answers[turn.payload.activity.id] : undefined} result={turn.payload.activity ? results[turn.payload.activity.id] : undefined} responding={respond.isPending} onAnswer={(activity, value) => { setAnswers(current => ({ ...current, [activity.id]: value })); setResults(current => { const next = { ...current }; delete next[activity.id]; return next }) }} onRespond={(activity, value) => respond.mutate({ activity, response: activity.options ? { choice: value } : activity.type === 'ordering' ? { order: value } : { text: value } })} onRefresh={() => sessionQuery.refetch()} />)}{send.isPending && <div className="flex items-center gap-3"><FlowCompanion state="thinking" className="w-14 animate-pulse" /><p className="text-sm font-bold text-flow-muted">Flow is thinking…</p></div>}{send.isError && <p role="alert" className="text-sm text-rose-300">That message didn’t reach Flow. Your earlier progress is safe—send it again.</p>}{session.status === 'mastery_check' && session.mastery >= 70 && <section className="border-l-4 border-flow-success pl-5"><p className="text-xl font-black">Yep. You’ve got this.</p><p className="mt-1 text-sm text-flow-muted">Your evidence is strong enough to secure the concept.</p><button onClick={() => complete.mutate()} disabled={complete.isPending} className="mt-4 min-h-12 bg-flow-success px-6 font-black text-flow-void">{complete.isPending ? 'Securing progress…' : 'Complete concept'}</button></section>}<div ref={endRef} /></div></main>
    <footer className="flow-v2 shrink-0 border-t border-white/10 bg-flow-void px-3 pb-[max(.75rem,env(safe-area-inset-bottom))] pt-3 sm:px-6"><div className="mx-auto flex max-w-3xl items-end gap-2"><button aria-label="Learning tools" className="grid h-12 w-12 shrink-0 place-items-center text-flow-muted"><Plus /></button><div className="flex min-h-12 flex-1 items-end border-b-2 border-white/20 focus-within:border-flow-orange"><textarea value={message} onChange={event => setMessage(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); submitMessage() } }} rows={1} placeholder="Message Flow…" className="max-h-32 flex-1 resize-none bg-transparent px-2 py-3 text-base outline-none" /><button onClick={submitMessage} disabled={!message.trim() || send.isPending} aria-label="Send message" className="grid h-12 w-12 place-items-center text-flow-orange disabled:opacity-30"><Send /></button></div><Link href={`/ai?teaching_session=${session.id}&concept=${node.id}&voice=1`} aria-label="Talk to Flow" className="grid h-12 w-12 shrink-0 place-items-center text-flow-violet"><Mic /></Link></div><div className="mx-auto mt-2 flex max-w-3xl gap-4 overflow-x-auto text-xs font-black text-flow-muted"><button onClick={() => send.mutate('show me a video')}>Video</button><button onClick={() => send.mutate('make flashcards from what we have learned')}>Flashcards</button><button onClick={() => send.mutate('give me one practice question')}>Practice</button><button onClick={() => send.mutate('show me the source')}>Source</button></div></footer></motion.div>
}
...
function ConversationTurn({ turn, node, answer, result, responding, onAnswer, onRespond, onRefresh }: { turn: TeachingTurn; node: WorldNode; answer: unknown; result?: EncounterAttemptResponse; responding: boolean; onAnswer: (activity: EncounterActivity, value: unknown) => void; onRespond: (activity: EncounterActivity, value: unknown) => void; onRefresh: () => void }) {
  ...
  return <section className="relative pl-14"><FlowCompanion state={turn.kind === 'completion' ? 'celebrating' : 'reading'} className="absolute left-0 top-0 w-10" /><div className="max-w-2xl"><MarkdownContent>{turn.content}</MarkdownContent>{activity && <div className="mt-5"><ActivityCard activity={activity} answer={answer} result={result} disabled={responding} onAnswer={value => onAnswer(activity, value)} onSubmit={() => onRespond(activity, answer)} /></div>}{turn.kind === 'video' && <VideoLearningObject videos={turn.payload.videos || []} />}{turn.kind === 'flashcards' && <FlashcardLearningObject nodeId={node.id} cards={turn.payload.cards || []} onSaved={onRefresh} />}</div></section>
}

function VideoLearningObject({ videos }: { videos: NonNullable<TeachingTurn['payload']['videos']> }) { return <div className="mt-4 grid gap-3 sm:grid-cols-2">{videos.map(video => <a key={video.video_id} href={video.url} target="_blank" rel="noreferrer" className="group border-l-4 border-flow-violet bg-white/[.035] p-4"><p className="font-black group-hover:text-flow-violet"><Play className="mr-2 inline h-4 w-4" />{video.title}</p><p className="mt-2 text-xs text-flow-muted">{video.channel}{video.duration_str ? ` · ${video.duration_str}` : ''}</p></a>)}</div> }
function FlashcardLearningObject({ nodeId, cards, onSaved }: { nodeId: string; cards: NonNullable<TeachingTurn['payload']['cards']>; onSaved: () => void }) { const [saved, setSaved] = useState(false); const save = useMutation({ mutationFn: () => learningApi.saveTeachingFlashcards(nodeId, cards), onSuccess: () => { setSaved(true); onSaved() } }); return <div className="mt-4"><div className="grid gap-3 sm:grid-cols-3">{cards.map((card, index) => <div key={index} className="min-h-36 border-t-4 border-flow-orange bg-white/[.035] p-4"><p className="text-xs font-black uppercase tracking-wide text-flow-orange">Card {index + 1}</p><p className="mt-3 text-sm font-bold">{card.question}</p><details className="mt-3 text-sm text-flow-muted"><summary>Reveal answer</summary><p className="mt-2">{card.answer}</p></details></div>)}</div><button onClick={() => save.mutate()} disabled={saved || save.isPending} className="mt-3 inline-flex min-h-11 items-center gap-2 text-sm font-black text-flow-violet"><Check className="h-4 w-4" />{saved ? 'Saved for review' : save.isPending ? 'Saving…' : 'Save these'}</button></div> }
