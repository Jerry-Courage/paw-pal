...
function StudyEncounter({ node, onClose, onCompleted }: { node: WorldNode; onClose: () => void; onCompleted: (reward: RewardResponse, mastery: number) => void }) {
  ...
  const progress = session.objectives.length ? Math.round((session.objectives_understood.length / session.objectives.length) * 100) : 0
  ...
  return <motion.div className="flow-v2 flow-atmosphere fixed inset-0 z-[60] flex flex-col text-flow-ink" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><header className="z-20 flex shrink-0 items-center gap-3 border-b border-white/[.07] bg-[#080914]/95 px-3 py-3 backdrop-blur sm:px-6"><button onClick={onClose} className="inline-flex min-h-11 items-center gap-2 text-sm font-black text-flow-muted"><ArrowLeft className="h-5 w-5" /><span className="hidden sm:inline">Journey</span></button><h1 className="min-w-0 flex-1 truncate text-sm font-black sm:text-lg">{normalizeReadableMath(node.title)}</h1><span className="text-xs font-black text-flow-muted">{progress}%</span></header><div className="h-1 shrink-0 bg-white/[.07]"><motion.div className="h-full bg-gradient-to-r from-flow-orange to-flow-violet" animate={{ width: `${progress}%` }} /></div>
    <main className="min-h-0 flex-1 overflow-y-auto overscroll-y-contain px-4 py-8 touch-pan-y lg:px-10"><div className="flow-teaching-grid"><div className="min-w-0 max-w-4xl space-y-8">{session.turns.map(turn => <ConversationTurn key={turn.id} turn={turn} node={node} answer={turn.payload.activity ? answers[turn.payload.activity.id] : undefined} result={turn.payload.activity ? results[turn.payload.activity.id] : undefined} responding={respond.isPending} onAnswer={(activity, value) => { setAnswers(current => ({ ...current, [activity.id]: value })); setResults(current => { const next = { ...current }; delete next[activity.id]; return next }) }} onRespond={(activity, value) => respond.mutate({ activity, response: activity.options ? { choice: value } : activity.type === 'ordering' ? { order: value } : { text: value } })} onRefresh={() => sessionQuery.refetch()} />)}{send.isPending && <div className="flex items-center gap-3"><FlowCompanion state="thinking" className="w-14" /><p className="text-sm font-bold text-flow-muted">Connecting the dots.</p></div>}{send.isError && <p role="alert" className="text-sm text-rose-300">That message didn’t reach Flow. Your earlier progress is safe—send it again.</p>}{session.completion_evaluation.complete && <section className="border-l-4 border-flow-success pl-5"><p className="text-xl font-black">Yep. You’ve got this.</p><p className="mt-1 text-sm text-flow-muted">The server has verified every required objective.</p><button onClick={() => complete.mutate()} disabled={complete.isPending} className="mt-4 min-h-12 bg-flow-success px-6 font-black text-flow-void">{complete.isPending ? 'Securing progress…' : 'Complete concept'}</button></section>}<div ref={endRef} /></div><aside className="sticky top-4 hidden h-fit border-l border-white/10 pl-6 lg:block"><FlowCompanion state={send.isPending ? 'thinking' : session.status === 'remediation' ? 'encouraging' : 'teaching'} className="mx-auto w-40" label="Flow teaching companion" /><p className="flow-eyebrow mt-2">Learning signal</p><p className="mt-2 text-3xl font-black">{session.completion_evaluation.objectives_satisfied}/{session.completion_evaluation.objectives_total}</p><p className="mt-1 text-sm text-flow-muted">objectives securely understood</p><div className="mt-6 space-y-3">{session.completion_evaluation.objectives.map(objective => <div key={objective.id} className="flex items-start gap-2 text-xs"><span className={cn('mt-1 h-2 w-2 shrink-0 rounded-full', objective.satisfied ? 'bg-flow-success' : objective.unresolved_misconception ? 'bg-rose-400' : 'bg-white/20')} /><span className={objective.satisfied ? 'text-flow-ink' : 'text-flow-muted'}>{objective.text}</span></div>)}</div></aside></div></main>
    <footer className="flow-v2 shrink-0 border-t border-white/[.07] bg-flow-void/95 px-3 pb-[max(.75rem,env(safe-area-inset-bottom))] pt-3 backdrop-blur sm:px-6"><div className="mx-auto flex max-w-3xl items-end gap-2"><button aria-label="Learning tools" className="grid h-12 w-12 shrink-0 place-items-center text-flow-muted"><Plus /></button><div className="flex min-h-12 flex-1 items-end border-b-2 border-white/20 focus-within:border-flow-orange"><textarea value={message} onChange={event => setMessage(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); submitMessage() } }} rows={1} placeholder="Message Flow…" className="max-h-32 flex-1 resize-none bg-transparent px-2 py-3 text-base outline-none" /><button onClick={submitMessage} disabled={!message.trim() || send.isPending} aria-label="Send message" className="grid h-12 w-12 place-items-center text-flow-orange disabled:opacity-30"><Send /></button></div><button onClick={() => setVoiceOpen(true)} aria-label="Talk to Flow" className="grid h-12 w-12 shrink-0 place-items-center text-flow-violet focus-visible:ring-2 focus-visible:ring-flow-orange"><Mic /></button></div><div className="mx-auto mt-2 flex max-w-3xl gap-4 overflow-x-auto text-xs font-black text-flow-muted"><button onClick={() => send.mutate('show me a video')}>Video</button><button onClick={() => send.mutate('make flashcards from what we have learned')}>Flashcards</button><button onClick={() => send.mutate('give me one practice question')}>Practice</button><button onClick={() => send.mutate('show me the source')}>Source</button></div></footer><AnimatePresence>{voiceOpen && <TeachingVoiceMode session={session} onClose={closeVoice} />}</AnimatePresence>{session.completion_evaluation.normal_requirements_met && !session.completion_evaluation.feynman.passed && <FeynmanFinale node={node} session={session} onRefresh={() => sessionQuery.refetch()} />}</motion.div>
}
...
function ConversationTurn({ turn, node, answer, result, responding, onAnswer, onRespond, onRefresh }: { turn: TeachingTurn; node: WorldNode; answer: unknown; result?: EncounterAttemptResponse; responding: boolean; onAnswer: (activity: EncounterActivity, value: unknown) => void; onRespond: (activity: EncounterActivity, value: unknown) => void; onRefresh: () => void }) {
  ...
  if (turn.role === 'learner') return <div className="ml-auto max-w-[82%] border-r-4 border-flow-orange pr-4 text-right"><p className="text-base font-bold">{turn.content || 'Response submitted'}</p></div>
  ...
  return <section className="relative pl-14"><FlowCompanion state={avatarState} className="absolute left-0 top-0 w-10" /><div className="max-w-2xl"><MarkdownContent>{turn.content}</MarkdownContent>{activity && <div className="mt-5"><ActivityCard activity={activity} answer={answer} result={result} disabled={responding} onAnswer={value => onAnswer(activity, value)} onSubmit={() => onRespond(activity, answer)} /></div>}{turn.kind === 'video' && <VideoLearningObject videos={turn.payload.videos || []} />}{turn.kind === 'flashcards' && <FlashcardLearningObject nodeId={node.id} cards={turn.payload.cards || []} onSaved={onRefresh} />}</div></section>
}

function VideoLearningObject({ videos }: { videos: NonNullable<TeachingTurn['payload']['videos']> }) { const [active, setActive] = useState(0); const video = videos[active]; if (!video) return null; return <figure className="mt-5 overflow-hidden border border-white/10 bg-[#0B0C1A]"><div className="aspect-video bg-black"><iframe className="h-full w-full" src={video.embed_url || `https://www.youtube-nocookie.com/embed/${video.video_id}?rel=0`} title={video.title} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen /></div><figcaption className="p-4"><p className="font-black text-flow-ink"><Play className="mr-2 inline h-4 w-4 text-flow-violet" />{video.title}</p><p className="mt-1 text-xs text-flow-muted">{video.channel}{video.duration_str ? ` · ${video.duration_str}` : ''}</p>{video.why && <p className="mt-3 text-sm text-flow-muted">{video.why}</p>}<div className="mt-3 flex items-center gap-4">{videos.length > 1 && <button onClick={() => setActive(index => (index + 1) % videos.length)} className="text-xs font-black text-flow-violet">Try another video</button>}<a href={video.url} target="_blank" rel="noreferrer" className="text-xs font-bold text-flow-muted underline">Open externally if playback is blocked</a></div></figcaption></figure> }
function FlashcardLearningObject({ nodeId, cards, onSaved }: { nodeId: string; cards: NonNullable<TeachingTurn['payload']['cards']>; onSaved: () => void }) { const [saved, setSaved] = useState(false); const [index, setIndex] = useState(0); const [flipped, setFlipped] = useState(false); const card = cards[index]; const save = useMutation({ mutationFn: () => learningApi.saveTeachingFlashcards(nodeId, cards), onSuccess: () => { setSaved(true); onSaved() } }); if (!card) return null; const move = (direction: number) => { setIndex(value => (value + direction + cards.length) % cards.length); setFlipped(false) }; return <section className="mt-5 max-w-xl" aria-label="Flow flashcards" onKeyDown={event => { if (event.key === 'ArrowRight') move(1); if (event.key === 'ArrowLeft') move(-1); if (event.key === ' ' || event.key === 'Enter') { event.preventDefault(); setFlipped(value => !value) } }} tabIndex={0}><div className="mb-3 flex items-center justify-between text-xs font-black uppercase tracking-[.16em] text-flow-muted"><span>Revision deck</span><span>{index + 1} / {cards.length}</span></div><button onClick={() => setFlipped(value => !value)} aria-pressed={flipped} className="relative min-h-64 w-full overflow-hidden border border-white/10 bg-gradient-to-br from-[#17182b] to-[#0B0C1A] p-7 text-left shadow-[0_18px_50px_rgba(0,0,0,.3)] focus-visible:ring-2 focus-visible:ring-flow-orange"><motion.div key={`${index}-${flipped}`} initial={{ rotateY: 75, opacity: 0 }} animate={{ rotateY: 0, opacity: 1 }} transition={{ duration: .24 }}><p className="text-xs font-black uppercase tracking-[.2em] text-flow-orange">{flipped ? 'Back' : 'Front'}</p><p className="mt-8 text-xl font-black leading-snug text-flow-ink">{flipped ? card.answer : card.question}</p><p className="mt-8 flex items-center gap-2 text-xs font-bold text-flow-muted"><RotateCw className="h-4 w-4" />Tap or press Space to flip</p></motion.div></button><div className="mt-3 flex items-center justify-between"><button onClick={() => move(-1)} aria-label="Previous flashcard" className="grid h-11 w-11 place-items-center text-flow-muted"><ChevronLeft /></button><button onClick={() => save.mutate()} disabled={saved || save.isPending} className="inline-flex min-h-11 items-center gap-2 text-sm font-black text-flow-violet"><Check className="h-4 w-4" />{saved ? 'Saved' : save.isPending ? 'Saving…' : 'Save cards'}</button><button onClick={() => move(1)} aria-label="Next flashcard" className="grid h-11 w-11 place-items-center text-flow-muted"><ChevronRight /></button></div>{saved && <p role="status" className="mt-2 text-center text-xs font-bold text-flow-success">Saved to your existing review deck.</p>}</section> }
