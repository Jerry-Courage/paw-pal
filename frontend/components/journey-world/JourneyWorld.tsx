...
import type { EncounterActivity, EncounterAttemptResponse, JourneyPathDetail, JourneyRoadmapNode, JourneyRoadmapResponse, JourneyStage, RewardResponse, TeachingPlan, TeachingSessionResponse, TeachingTurn } from '@/types/journey'
...
function StageMoment({ stage, node, plan, answer, result, responding, continuing, onContinue, onAnswer, onRespond, onRefresh }: { stage: JourneyStage; node: WorldNode; plan?: TeachingPlan | null; answer: unknown; result?: EncounterAttemptResponse; responding: boolean; continuing: boolean; onContinue: () => void; onAnswer: (activity: EncounterActivity, value: unknown) => void; onRespond: (activity: EncounterActivity, value: unknown) => void; onRefresh: () => void }) {
  ...
  const line = result?.correct === true ? 'Yep. That clicks.' : result?.correct === false ? 'Close. Watch what changes.' : stage.type === 'FLOW_INTRO' ? 'Ready? One idea at a time.' : evidenceStage ? 'Your turn.' : activity?.type === 'process' ? 'Watch what happens inside.' : activity?.type === 'comparison' ? 'Keep these two apart.' : activity?.type === 'worked_example' ? 'Let’s work it through.' : 'Look here.'
  ...
  return <motion.section className="w-full py-2" initial={{ opacity: 0, x: 28, scale: .98 }} animate={{ opacity: 1, x: 0, scale: 1 }} exit={{ opacity: 0, x: -28, scale: .98 }} transition={{ duration: .3, ease: 'easeOut' }} aria-label="Current learning moment">
    <FlowReaction state={reaction} line={line} position={evidenceStage ? 'edge' : activity?.type === 'worked_example' ? 'beside' : 'upper'} className="mb-2" />
    <div className="mx-auto w-full max-w-[900px] px-0 sm:px-5">
      ...
      {activity && <ActivityCard activity={activity} answer={answer} result={result} historical={false} disabled={responding} onAnswer={value => onAnswer(activity, value)} onSubmit={() => onRespond(activity, answer)} />}
    ...
    {!evidenceStage && <div className="mx-auto mt-5 flex max-w-[800px] justify-center"><button onClick={onContinue} disabled={continuing} className="min-h-12 rounded-full bg-flow-orange px-7 font-black text-flow-void shadow-[0_4px_0_#8f3600] transition hover:-translate-y-0.5 active:translate-y-1 active:shadow-none disabled:opacity-50">{continuing ? 'One sec…' : action}</button></div>}
  </motion.section>
}
