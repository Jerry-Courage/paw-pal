...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, BookOpen, Check, ChevronLeft, ChevronRight, Clock3, Coins, Crown, Flame, Headphones, Lightbulb, LockKeyhole, Menu, MessageCircle, Mic, Play, Plus, RefreshCw, RotateCw, Send, ShieldCheck, Swords, Target, X } from 'lucide-react'
...
import type { EncounterActivity, EncounterAttemptResponse, JourneyPathDetail, JourneyRoadmapNode, JourneyRoadmapResponse, JourneyStage, RewardResponse, TeachingSessionResponse, TeachingTurn } from '@/types/journey'
...
function StudyEncounter({ node, onClose, onCompleted }: { node: WorldNode; onClose: () => void; onCompleted: (reward: RewardResponse, mastery: number) => void }) {
  ...
  return <motion.div className="flow-v2 flow-atmosphere fixed inset-0 z-[60] flex min-w-0 flex-col overflow-hidden text-flow-ink" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><header className="z-20 shrink-0 border-b border-white/[.07] bg-flow-void/95 px-3 pb-3 pt-[max(.5rem,env(safe-area-inset-top))] backdrop-blur sm:px-6"><div className="mx-auto flex max-w-[900px] items-center gap-3"><button onClick={onClose} aria-label="Back to Journey" className="grid h-11 w-11 shrink-0 place-items-center text-flow-muted hover:text-flow-ink focus-visible:ring-2 focus-visible:ring-flow-orange"><ArrowLeft className="h-5 w-5" /></button><div className="min-w-0 flex-1"><div className="flex items-center gap-3"><h1 className="min-w-0 flex-1 truncate text-sm font-black sm:text-base">{normalizeReadableMath(node.title)}</h1><button onClick={() => setProgressOpen(true)} aria-label={`Open Journey progress, ${progress}% complete`} className="shrink-0 text-xs font-black text-flow-muted hover:text-flow-ink focus-visible:ring-2 focus-visible:ring-flow-orange">{progress}%</button></div><div className="mt-2 flex gap-1" aria-label={`${satisfiedCount} of ${objectiveState.length} checkpoints understood`}>{objectiveState.map((objective, index) => <span key={objective.id} className={cn('h-1.5 min-w-2 flex-1 rounded-full transition-colors', objective.satisfied ? 'bg-flow-success' : index === currentObjectiveIndex ? 'bg-flow-orange' : 'bg-white/10')} />)}</div><p className="mt-2 line-clamp-2 text-[11px] font-bold leading-snug text-flow-muted"><span className="mr-2 uppercase tracking-[.14em] text-flow-orange">Now</span>{normalizeReadableMath(currentCheckpointTitle)}</p></div></div></header>
    <main className="min-h-0 flex-1 overflow-y-auto overscroll-y-contain px-4 py-5 touch-pan-y sm:px-6 sm:py-7"><div className="mx-auto flex min-h-full w-full max-w-[860px] flex-col"><section className="mb-5 flex items-center justify-between gap-3 border-b border-white/10 pb-4"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-flow-orange">Checkpoint {Math.min(currentObjectiveIndex + 1, objectiveState.length)} of {objectiveState.length}</p><h2 className="mt-1 text-lg font-black leading-snug sm:text-xl">{normalizeReadableMath(currentCheckpointTitle)}</h2></div><span className="text-xs font-bold text-flow-muted">~{Math.max(2, Math.ceil(node.estimated_minutes / Math.max(1, objectiveState.length)))} min</span></section><AnimatePresence>{checkpointNotice && <motion.div role="status" initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="mb-5 flex items-center gap-3 border-l-2 border-flow-success py-2 pl-4"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-flow-success text-flow-void"><Check className="h-4 w-4" /></span><p className="text-sm font-bold text-flow-ink">Objective cleared · {normalizeReadableMath(checkpointNotice)}</p></motion.div>}</AnimatePresence><div className="flex flex-1 items-center"><AnimatePresence mode="wait">{activeStage && <StageMoment key={activeStage.id} stage={activeStage} node={node} answer={activeStage.payload.activity ? answers[activeStage.payload.activity.id] : undefined} result={activeStage.payload.activity ? results[activeStage.payload.activity.id] : undefined} responding={respond.isPending} continuing={continueStage.isPending} onContinue={() => continueStage.mutate(activeStage.id)} onAnswer={(activity, value) => { setAnswers(current => ({ ...current, [activity.id]: value })); setResults(current => { const next = { ...current }; delete next[activity.id]; return next }) }} onRespond={(activity, value) => respond.mutate({ activity, response: activity.options ? { choice: value } : activity.type === 'ordering' ? { order: value } : { text: value } })} onRefresh={() => sessionQuery.refetch()} />}</AnimatePresence></div>{send.isPending && <div className="mt-4 flex items-center gap-3"><FlowCompanion state="thinking" className="w-12" /><p className="text-sm font-bold text-flow-muted">Connecting the dots.</p></div>}{send.isError && <p role="alert" className="mt-4 text-sm text-rose-300">That message didn’t reach Flow. Your stage and progress are safe.</p>}{session.completion_evaluation.complete && <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mt-5 border-y border-flow-success/30 py-6 text-center"><span className="mx-auto grid h-10 w-10 place-items-center rounded-full bg-flow-success text-flow-void"><Check /></span><p className="mt-3 text-xl font-black">Objective cleared.</p><button onClick={() => complete.mutate()} disabled={complete.isPending} className="mt-5 min-h-12 bg-flow-success px-6 font-black text-flow-void">{complete.isPending ? 'Securing progress…' : 'Finish this concept'}</button></motion.section>}<div ref={endRef} /></div></main>
...
function StageMoment({ stage, node, answer, result, responding, continuing, onContinue, onAnswer, onRespond, onRefresh }: { stage: JourneyStage; node: WorldNode; answer: unknown; result?: EncounterAttemptResponse; responding: boolean; continuing: boolean; onContinue: () => void; onAnswer: (activity: EncounterActivity, value: unknown) => void; onRespond: (activity: EncounterActivity, value: unknown) => void; onRefresh: () => void }) {
  ...
  const line = result?.correct === true ? 'Yep — that holds. Nice work.' : result?.correct === false ? 'Almost. Let’s find the bit that slipped.' : stage.type === 'FLOW_INTRO' ? 'Let’s make this one click.' : evidenceStage ? 'Your turn. What do you notice?' : undefined
  return <motion.section className="w-full py-2" initial={{ opacity: 0, x: 36, scale: .97 }} animate={{ opacity: 1, x: 0, scale: 1 }} exit={{ opacity: 0, x: -36, scale: .97 }} transition={{ duration: .38, ease: 'easeOut' }} aria-label="Current learning moment">
    <FlowReaction state={reaction} line={line} className="mb-5" />
    <div className="mx-auto w-full max-w-[800px] px-0 sm:px-5">
      {stage.type === 'FLOW_INTRO' && <div className="py-6 text-center"><p className="text-[clamp(1.5rem,4vw,2.75rem)] font-black leading-tight tracking-[-.04em]">{normalizeReadableMath(stage.payload.text || stage.title)}</p><p className="mx-auto mt-4 max-w-lg text-sm leading-6 text-flow-muted">We’ll build this one idea at a time.</p></div>}
    ...
    {!evidenceStage && <div className="mx-auto mt-7 flex max-w-[800px] justify-center sm:justify-end"><button onClick={onContinue} disabled={continuing} className="min-h-14 rounded-full bg-flow-orange px-8 font-black text-flow-void shadow-[0_5px_0_#8f3600] transition hover:-translate-y-0.5 active:translate-y-1 active:shadow-none disabled:opacity-50">{continuing ? 'Moving on…' : 'Got it'}</button></div>}
  </motion.section>
}
...
function ActivityCard({ activity, answer, result, historical, disabled, onAnswer, onSubmit }: { activity: EncounterActivity; answer: unknown; result?: EncounterAttemptResponse; historical: boolean; disabled: boolean; onAnswer: (value: unknown) => void; onSubmit: () => void }) {
  ...
  return <article className="py-2">{diagnostic && <div className="mb-5 flex items-start gap-3 rounded-2xl bg-flow-violet/10 p-4"><Target className="mt-0.5 h-5 w-5 shrink-0 text-flow-violet" /><div><p className="font-black text-flow-violet">A quick warm-up</p><p className="mt-1 text-sm text-flow-muted">This helps Flow choose where to begin. It won’t count against you.</p></div></div>}{!remediation && !teaching && <p className="mx-auto max-w-3xl text-[clamp(1.25rem,2.3vw,1.9rem)] font-black leading-snug tracking-[-.025em] text-flow-ink">{normalizeReadableMath(activity.prompt)}</p>}{activity.instructions && <p className="mx-auto mt-3 max-w-3xl text-sm text-flow-muted">{activity.instructions}</p>}<SourceProvenance activity={activity} />
    {remediation ? <FocusedRemediationActivity activity={activity} /> : teaching ? <JourneyLearningObject object={activity} /> : activity.type === 'ordering' ? <OrderingInteraction activity={activity} answer={answer} result={result} onAnswer={onAnswer} /> : activity.type === 'matching' ? <MatchingInteraction activity={activity} answer={answer} result={result} onAnswer={onAnswer} /> : activity.type === 'sorting' ? <SortingInteraction activity={activity} answer={answer} result={result} onAnswer={onAnswer} /> : activity.type === 'tap_target' ? <TapTargetInteraction activity={activity} answer={answer} result={result} onAnswer={onAnswer} /> : activity.type === 'evidence_highlight' ? <EvidenceHighlightInteraction activity={activity} answer={answer} result={result} onAnswer={onAnswer} /> : activity.type === 'step_solver' ? <StepSolverCanvas object={activity} onAnswer={onAnswer} /> : activity.type === 'reveal' ? <RevealInteraction activity={activity} /> : activity.options?.length ? <ChoiceActivity activity={activity} answer={answer} result={result} onAnswer={onAnswer} /> : <ConversationalAnswer activity={activity} answer={answer} result={result} onAnswer={onAnswer} />}
    {!historical && !teaching && activity.type !== 'reveal' && (result ? <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} className={cn('mt-5 rounded-2xl p-4 text-sm', result.correct === false ? 'bg-rose-400/10' : 'bg-flow-success/10')}><p className="font-black text-flow-ink">{result.correct === false ? result.outcome === 'partial' ? 'Close — one piece is missing.' : 'Not quite. Here’s what to notice.' : result.correct === null ? 'Got it.' : 'Exactly.'}</p><MarkdownContent>{result.feedback}</MarkdownContent>{result.correct === false && <button onClick={() => onAnswer(undefined)} className="mt-3 min-h-11 font-black text-flow-orange">Try again</button>}</motion.div> : <div className="mt-6 flex justify-center sm:justify-end"><button disabled={!canSubmitInteraction(activity, answer) || disabled} onClick={onSubmit} className="min-h-14 rounded-full bg-flow-violet px-7 font-black text-flow-void shadow-[0_5px_0_#5140a8] active:translate-y-1 active:shadow-none disabled:opacity-40">{disabled ? 'Checking…' : activity.type === 'ordering' ? 'Check order' : 'Check'}</button></div>)}
  </article>
}
...
function SourceProvenance({ activity }: { activity: EncounterActivity }) {
  if (['check', 'apply', 'transfer'].includes(activity.purpose)) return <p className="mt-3 text-[10px] font-black uppercase tracking-[.16em] text-flow-violet">Quick check · checkpoint {(activity.objective_index ?? 0) + 1}</p>
