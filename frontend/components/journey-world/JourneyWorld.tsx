...
function ActivityCard({ activity, answer, result, historical, disabled, onAnswer, onSubmit }: { activity: EncounterActivity; answer: unknown; result?: EncounterAttemptResponse; historical: boolean; disabled: boolean; onAnswer: (value: unknown) => void; onSubmit: () => void }) {
  ...
  return <article className={cn('border-y border-white/10 py-5 sm:py-7', historical && 'opacity-55')}>{diagnostic && <div className="mb-5 flex items-start gap-3 border-l-4 border-flow-violet pl-4"><Target className="mt-0.5 h-5 w-5 shrink-0 text-flow-violet" /><div><p className="text-xs font-black uppercase tracking-[.16em] text-flow-violet">Quick diagnostic</p><p className="mt-1 text-sm text-flow-muted">This first question helps Flow choose the best route. It won’t count against your mastery.</p></div></div>}{!remediation && <p className="text-[clamp(1.2rem,2.2vw,1.75rem)] font-black leading-snug tracking-[-.02em] text-flow-ink">{normalizeReadableMath(activity.prompt)}</p>}{activity.instructions && <p className="mt-3 text-sm text-flow-muted">{activity.instructions}</p>}<SourceProvenance activity={activity} />{historical && <p className="mt-4 text-xs font-black uppercase tracking-[.14em] text-flow-muted">Previous checkpoint · read only</p>}
    {!historical && (remediation ? <FocusedRemediationActivity activity={activity} /> : activity.type === 'comparison' && activity.content?.rows ? <ComparisonActivity activity={activity} /> : activity.type === 'worked_example' ? <WorkedExampleActivity activity={activity} /> : activity.type === 'ordering' ? <OrderingActivity activity={activity} answer={answer} result={result} onAnswer={onAnswer} /> : activity.options?.length ? <ChoiceActivity activity={activity} answer={answer} result={result} onAnswer={onAnswer} /> : <ConversationalAnswer activity={activity} answer={answer} result={result} onAnswer={onAnswer} />)}
...
function FocusedRemediationActivity({ activity }: { activity: EncounterActivity }) {
  ...
  return <section className="border-l-4 border-flow-orange py-1 pl-4 sm:pl-5"><p className="text-[10px] font-black uppercase tracking-[.18em] text-flow-orange">Different angle</p><h3 className="mt-2 text-xl font-black text-flow-ink">{content?.title || activity.prompt}</h3>{content?.lead && <p className="mt-2 text-sm leading-6 text-flow-muted">{content.lead}</p>}{content?.columns && content.rows ? <ComparisonActivity activity={activity} /> : content?.steps?.length ? <ol className="mt-5 space-y-3">{content.steps.map((step, index) => <li key={index} className="flex gap-3 text-sm leading-6 text-flow-muted"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-flow-orange/15 font-black text-flow-orange">{index + 1}</span><span>{normalizeReadableMath(step)}</span></li>)}</ol> : <MarkdownContent>{content?.example || ''}</MarkdownContent>}{content?.takeaway && <div className="mt-5 border-t border-white/10 pt-4"><p className="text-[10px] font-black uppercase tracking-[.16em] text-flow-violet">Keep this distinction</p><MarkdownContent>{content.takeaway}</MarkdownContent></div>}</section>
}
